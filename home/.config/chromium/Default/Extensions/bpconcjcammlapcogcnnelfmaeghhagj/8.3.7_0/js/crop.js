(function ($) {
    ...
    function showLoupe(e) {
        ...
        $(s[0]).html('X = ' + e.clientX * z);
        $(s[1]).html('Y = ' + e.clientY * z);
    ...
    function cropImage(save) {
        ...
        if (save) {
            ...
            imgdata = data;
//            chrome.extension.sendMessage({msg: 'get_file_name', pageinfo: {'url': document.URL, 'title': document.title}}, function (response) {
//                $('#imgdownload').attr('download', response + '.' + ((format == 'jpeg') ? 'jpg' : format));
//            });
        } else {
    ...
    function createCoords(c) {
        ...
        var btnd = $('<button/>', {
            ...
            'id':    'imgdownload',
            'class': 'edit_btn save',
            'title': chrome.i18n.getMessage("cropBtnSave")
        });
...
//        });
        ...
        btnd.bind('click', function () {
            chrome.extension.sendMessage({msg: 'save_image', data: imgdata}, function (response) {
                console.log(response);
            });
            destroyCrop();
        });
        ...
        btnnimbus.bind('click', function () {
            chrome.extension.sendMessage({msg: 'sendtonimbus', img: imgdata}, function (response) {
                console.log(response);
            });
            destroyCrop();
        });
        ...
        $('.edit_btn').hover(function () {
            $(".name", this).stop().animate({top: '32px', bottom: '0px'}, {queue: false, duration: 160});
        }, function () {
