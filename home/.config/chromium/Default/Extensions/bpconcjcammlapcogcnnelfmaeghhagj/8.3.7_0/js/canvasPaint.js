...
//                canvas.cache.data = canvas.fon.context.getImageData(0, 0, canvas.common.width, canvas.common.height);
            ...
            setButton:      function () {
                if (this.index > 0) {
                    $('#undo').removeClass('inert');
                    $('#undo-all').removeClass('inert');
                } else {
                    $('#undo').addClass('inert');
                }
                if (this.index < this.max) {
                    $('#redo').removeClass('inert');
                } else {
                    $('#redo').addClass('inert');
                }
            }
        ...
        function setFontProperty(size, family) {
            ...
            if (family) {
                $('#font .fml').text(family).css({'font-family': family});
            }

        }

        function showFontSettings(param) {
            var line = $('.options_tools .tools_line_width');
            var font = $('.options_tools .tools_font');
        ...
        var ObTools = {
            ...
            setCursor:       function (x, y) {
                ...
                if (this.marker > -1) {
                    ...
                    if (this.type === 'sticker' && this.marker === 0) {
                        canva.css("cursor", "url(css/rotate.png) 18 18, auto");
                    }
                }
            ...
            drawNumber:      function () {
                if (enablenumbers) {
                    ...
                    this.context.font = "38px Fantasy";
        ...
        function ObRectangle() {
            ...
            this.getNumberPosition = function () {
                return {x: Math.min(this.x, this.x + this.w) + this.number.x - 15, y: Math.min(this.y, this.y + this.h) + this.number.y - 20}
            }
        }
        ...
        function ObEllipse() {
            ...
            this.getNumberPosition = function () {
                return {x: Math.min(this.x, this.x + this.w) + this.number.x - 10, y: Math.min(this.y, this.y + this.h) + Math.abs(this.h / 4) + this.number.y}
            }
        }
...
//                if (this.text === '') this.text = 'еntеr thе tеxt';
                ...
                var canvas = document.createElement('canvas');
                canvas.width = this.w + 20;
                canvas.height = this.h + 23;
...
//                    if (this.text === 'еntеr thе tеxt') this.text = '';
            ...
            this.drawMarks = function () {
                this.drawMark(this.x - 5, this.y - 5);
                this.drawMark(this.x - 5, this.y + this.h + 5);
                this.drawMark(this.x + this.w + 5, this.y - 5);
                this.drawMark(this.x + this.w + 5, this.y + this.h + 5);

                this.context.beginPath();
                this.context.strokeStyle = '#999';
                this.context.moveTo(this.x + 0.5, this.y + 0.5);
                this.context.lineTo(this.x + 0.5, this.y + this.h + 0.5);
                this.context.lineTo(this.x + this.w + 0.5, this.y + this.h + 0.5);
                this.context.lineTo(this.x + this.w + 0.5, this.y + 0.5);
            ...
            this.checkMarkers = function (x, y) {
                this.marker = -1;
                if (this.checkMarker(x, y, this.x - 5, this.y - 5)) this.marker = 0;
                if (this.checkMarker(x, y, this.x + this.w + 5, this.y - 5)) this.marker = 1;
                if (this.checkMarker(x, y, this.x + this.w + 5, this.y + this.h + 5)) this.marker = 2;
                if (this.checkMarker(x, y, this.x - 5, this.y + this.h + 5)) this.marker = 3;
            };
            this.getNumberPosition = function () {
                return {x: Math.min(this.x, this.x + this.w) + this.number.x - 15, y: Math.min(this.y, this.y + this.h) + this.number.y - 20}
            };
        ...
        function ObTextArrow() {
            ...
            this.saveStore = function () {
                ...
                var l = this.store.length;
                this.store.splice(this.index, l - this.index);
                this.store.push({x: this.x, y: this.y, x2: this.x2, y2: this.y2, xb1: this.xb1, yb1: this.yb1, xb2: this.xb2, yb2: this.yb2, t: this.text});
            };
            this.readStore = function () {
                if (this.index > -1 && this.index < this.store.length) {
                    var s = this.store[this.index];
        ...
        var drawStart = function (event) {
            ...
            switch (state.tool) {
                ...
                case TOOLS.HIGHLIGHT:
                    canvas.current.context.lineWidth = 15;
            ...
            if ('setValue' in currentObject && state.tool !== TOOLS.EDIT) {
                if (state.tool !== TOOLS.STICKER && state.tool !== TOOLS.TEXT_ARROW) {
                    currentObject.number.n = getNumber();
                }
...
//                        currentObject.draw(canvas.current.context);
        ...
        var drawCursor = function (event) {
            ...
            if (!state.drawing) {
                ...
                if (state.tool === TOOLS.EDIT) {
                    if (!state.move) {
                        objects[currentN].setCursor(x, y);
                    }
                } else {
...
//            resetHistory();
        ...
        var checkObject = function (x, y) {
            $('#editcanva').css("cursor", getDefaultCursor());
            currentN = -1;
            var l, n = objects.length - 1;
            for (l = n; l >= 0; l--) {
        ...
        var getDefaultCursor = function () {
            ...
            if (state.tool === TOOLS.PEN || state.tool === TOOLS.HIGHLIGHT) {
                return "url(css/pencil.cur), auto";
            }
            return "default";
        };
