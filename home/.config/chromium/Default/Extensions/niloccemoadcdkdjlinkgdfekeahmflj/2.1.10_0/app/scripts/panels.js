...
(function (PKT_EXT) {
    ...
    var startScrollCloseTimer = function startScrollCloseTimer(e) {
        var distanceScrolled = Math.abs($(document).scrollTop() - initalPosition);
        if (distanceScrolled < 100 || mouseInside) return;
        window.removeEventListener('scroll', startScrollCloseTimer);
        startCloseTimer(500);
    ...
    var init = function init(urlToSave, itemID, features) {
    ...
    var showSave = function showSave(urlToSave, itemID) {

        PKT_EXT.TOOLBAR.setMessage(PKT_EXT.TRANS['page_saved']);
        ...
        if (!optionRecs) initAutoCloseEvents(5000);

        mainContainer.classList.remove('pkt_ext_container_inactive');
    };

    var saveURL = function saveURL(urlToSave, itemID, features) {

        if (!interfaceBuilt) init(urlToSave, itemID, features);else showSave(urlToSave, itemID);
    };
...
(function (PKT_EXT) {
    ...
    PKT_EXT.TIMER = { start: start, stop: stop, render: render };
})(PKT_EXT || {});
...
(function (PKT_EXT, taglib) {
    ...
    var sanitizeText = function sanitizeText(s) {
        var sanitizeMap = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': '&quot;', "'": '&#39;' };
        if (typeof s !== 'string') return '';else return String(s).replace(/[&<>"']/g, function (str) {
    ...
    var removeAction = function removeAction(el) {
        return el.removeEventListener('click', function (evt) {
            return tagObject.add(el.textContent);
        });
    };
    ...
    var initTagInput = function initTagInput(placeholder, container) {
        tagObject = new taglib(container, {
            ...
            onEmptySubmit: function onEmptySubmit(event) {
                closePanel();
            }
        });
