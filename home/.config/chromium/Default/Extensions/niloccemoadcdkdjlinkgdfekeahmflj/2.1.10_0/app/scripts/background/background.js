/* globals $, PKT_EXT, isChromeOnly, chrome, sendMessageToTab, authentication, broadcastMessageToAllTabs,  isValidURL,  addMessageListener, getSetting, setSetting, openTabWithURL, isMac */
...
===========================================================================*/
    function onSaveSuccess(tab, showToolbarIcon, itemId) {
        ...
        chrome.tabs.sendMessage(tab.id, {status: 'success', item_id: itemId, features: featureSet })
    }
        ...
        switch(request.action){
            ...
            case 'getSetup':
                ...
                if(!setupForce && typeof getSetting('setup') !== 'undefined'){
                    ...
                    if(presetSetup && presetSetup.new_tab){
                        sendResponse({ status: 'success', setup: presetSetup})
                        return true
                    } else {
                        var new_tab_pulse = getSetting('new_tab_pulse')
                        if(typeof new_tab_pulse === 'undefined'){
                            setSetting('new_tab_pulse', Date.now())
                            setSetting('new_tab_pulse_delay', 43200000)
                            // sendResponse({ status: 'success', setup: presetSetup})
                            // return true
                        } else if(Date.now() - new_tab_pulse < getSetting('new_tab_pulse_delay')){
                            log.debug('Checking setup in: ', ((getSetting('new_tab_pulse_delay') - (Date.now() - new_tab_pulse))/60000) + ' minutes')
                            sendResponse({ status: 'success', setup: presetSetup})
                            return true
                        } else {
                            log.debug('Checking for setup')
                            setSetting('new_tab_pulse', Date.now())
                        }
                    }
                }
